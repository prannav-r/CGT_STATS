...
def process_scorecard(image_path, is_batting):
    ...
    try:
        ...
        # Calculate bonus points for this match
        for player_stat in data:
            # Individual achievement bonuses for this match
            match_bonus = 0
            if is_batting:
            ...
            # Combined achievement bonuses for this match
            ...
            # Add match bonus to total FPL
            player_stat['match_fpl'] = player_stat['fpl'] + match_bonus

        # Load existing fantasy teams
        fantasy_teams = []
        try:
        ...
        # Update fantasy points for all teams with cumulative points
        for team in fantasy_teams:
            for player_name in team:
                if player_name == 'Team_Name':
                    continue
                # Look for player's points in this match from both batting and bowling data
                match_points = 0
                for player_stat in data:
                    if player_name.strip() == player_stat[player_key].strip():
                        match_points += player_stat['match_fpl']
                for comp_stat in complementary_data:
                    if player_name.strip() == comp_stat[check_key].strip():
                        match_points += comp_stat.get('match_fpl', 0)
                
                # Add match points to existing cumulative points
                team[player_name] = team.get(player_name, 0) + match_points
...
def save_team(tname, players):
    ...
    # Initialize points as 0 for new teams
